...
<div class="wrapper row2">
  <div id="container"> 
    <center>
	<h1>DOCTOR VIEW PATIENTS WITH LAB REPORT(S)</h1>
    <p>
	<?php 
	include"connection.eno";
	error_reporting(E_ERROR);
	$query = mysqli_query($con,"select * from patientdetails where labdoctor='1' && status='1'");
	...
	while($data = mysqli_fetch_array($query))
	{
		...
		<tr>
			...
			<td>'.$data['email'].'</td>
			<td>
				<form action="viewlabresults.php" method="post">
				<input type="hidden" value="'.$data['username'].'" name="username"/>
				<input type="submit" value="View Lab Results" style="width:100%;background-color:green;color:white;"/>
				</form>
			</td>
		</tr>
		';
